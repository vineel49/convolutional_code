...
% Get trellis
[Prev_State,Prev_Ip,Outputs_prev] = Get_Trellis_manual();  
...
[path_metric(:,sym_cnt+1),index] = min([path_metric(Prev_State(:,1),sym_cnt)+ branch_metric(Outputs_prev(:,1),sym_cnt) path_metric(Prev_State(:,2),sym_cnt)+ branch_metric(Outputs_prev(:,2),sym_cnt)],[],2);
