...
% Get trellis
[P_State,P_Ip,Ga_Inx] = Get_Trellis_manual();  
...
P_State_trans = P_State'; %transpose of P_State matrix - see trellis.
P_Ip_trans = P_Ip'; %transpose of P_Ip matrix - see trellis.
index_temp = [0;2;2*2;3*2]; %for ip=2; for linear indexing.

% Algorithm starts

for sym_cnt=  1:frame_size  
[path_metric(:,sym_cnt+1),index] = min([path_metric(P_State(:,1),sym_cnt)+ branch_metric(Ga_Inx(:,1),sym_cnt) path_metric(P_State(:,2),sym_cnt)+ branch_metric(Ga_Inx(:,2),sym_cnt)],[],2);
